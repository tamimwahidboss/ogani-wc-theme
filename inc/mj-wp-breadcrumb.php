<?php
    ...
    if ( ! function_exists( 'mj_wp_breadcrumb' ) ) {
        function mj_wp_breadcrumb ( $list_style = 'ol', $list_id = 'breadcrumb', $list_class = 'breadcrumb__text', $active_class = 'active', $echo = true ) {
            // Get text domain for translations
            $theme = wp_get_theme();
            $text_domain =  $theme->get( 'ogani' );
            ...
            // Front page
            // if ( is_front_page() ) {
            //     $breadcrumb .= '<h2 class="breadcrumb__option ' . $active_class . '">' . get_bloginfo('name') . '</h2>';
            // } else {
            //     $breadcrumb .= '<li class="breadcrumb__option"><a href="' . home_url() . '">' . get_bloginfo( 'name' ) . '</a></li>';
            // }

            // Blog archive
            if ( 'page' == get_option( '    ' ) && get_option( 'page_for_posts' ) ) {
                $blog_page_id = get_option( 'page_for_posts' );

                if ( is_home() ) {
                    $breadcrumb .= '<h2 class="' . $active_class . '">' . get_the_title( $blog_page_id )  . '</h2>';
            ...
            if ( is_archive() && ! is_tax() && ! is_post_type_archive() ) {
                $breadcrumb .= '<h2 class="' . $active_class . '">';
            ...
            if ( is_singular( 'post' ) ) {
                ...
                // Post title
                $breadcrumb .= '<h2 class="' . $active_class . '">' . get_the_title() . '</h2>';
            }
            ...
            // Search
            if ( is_search() ) {
                $breadcrumb .= '<h2 class="' . $active_class . '">' . __( 'Search', $text_domain ) . '</h2>';
            }

            // 404
            if ( is_404() ) {
                $breadcrumb .= '<h2 class="' . $active_class . '">' . __( '404', $text_domain ) . '</h2>';
            }

            // Custom Post Type Archive, shop page
			if ( is_post_type_archive() ) {
				$breadcrumb .= '<h2 class="' . $active_class . '">' . post_type_archive_title( '', false ) . '</h2>';
			}
			...
			if ( is_tax() ) {
				...
				// Term title, product categories
				$breadcrumb .= '<h2 class="' . $active_class . '">' . single_term_title( '', false ) . '</h2>';
			}
